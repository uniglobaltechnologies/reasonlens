...
**Live**: `https://purple-hill-0a1de9703.1.azurestaticapps.net`
...
| `/framework-recommender` | POST | None | Tool-calling LLM maps quiz answers to framework recommendations. |
| `/task-evaluator` | POST | None | Scores educational tasks for AI feasibility (1-5). Returns augment/automate/avoid. |
...
| Endpoint | Method | Auth | Description |
|---|---|---|---|
| `/run-petri-audit` | POST | JWT + runner/admin | Launches PETRI safety audit fire-and-forget |
...
| `/policy` | Policy Generator | 3-step wizard |
...
| `PETRI_SERVICE_URL` | Yes | PETRI Container App endpoint |
| `PETRI_CALLBACK_SECRET` | Yes | HMAC secret for callback validation |
| `TOXICITY_SERVICE_URL` | Yes | Posthoc toxicity scoring service URL |
| `BENCHMARK_SERVICE_URL` | Yes | Benchmark runner service URL |
...
| # | Item | Status |
|---|---|---|
| 1 | All 16 API functions | Done |
| 2 | All 14 frontend routes | Done |
...
| 8 | Set Azure Function App env vars | Pending — config only |
| 9 | Set GitHub Actions secrets | Pending — config only |
| 10 | Configure PETRI_CALLBACK_SECRET on PETRI service | Pending — config only |
| 11 | Configure TOXICITY_SERVICE_URL + BENCHMARK_SERVICE_URL | Pending — config only |
| 12 | Custom domain on Static Web App | Pending — config only |
